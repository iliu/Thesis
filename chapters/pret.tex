...
However, by using a TDMA schedule to predictably handle the structural hazards, the temporal isolation of threads enable a much tighter and simpler WCET analysis~\todo{cite}.
...
This phenomenon is known as \emph{cache thrashing}~\todo{cite}.
Partitioned caches~\todo{cite} in this case can be used to enforce separate internal states, so each thread updates only its own internal state.  
...
Side-channel attacks on encryption algorithms\todo{cite} exploit the shared hardware states to disrupt and probe the execution time of threads running the encryption algorithm.
...
\begin{center}
\noindent\makebox[\textwidth]{%
\includegraphics[scale=.58]{figs/branch_execution_compare}}
\end{center}
\vspace{-3mm}
...
\begin{center}
\noindent\makebox[\textwidth]{%
\includegraphics[scale=.6]{figs/data_hazard_execution_compare}}
\end{center}
\vspace{-3mm}
