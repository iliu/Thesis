...
A timing based communication and scheme is used to reduce communication overhead.
...
We present the synthesis results to show that by using the PRET architecture we can successfully simulate a common fuel rail configuration of up to 234 nodes on a Xilinx Virtex-6 FPGA, and enable a real-time engine fuel rail simulation.  
...
The floating point divide and square root are the most resource intensive hardware units, but the valve and ``T'' nodes usually represent only a few percent of the overall system.
...
Instead, by allowing different configurations of the PTARM cores within the system, we can include the extra hardware units only on cores that require them, getting the performance boost from hardware without a huge resource overhead.   
...
Each \delayuntil\ macro takes at least 2 thread cycles because it manipulates a 64-bit value representing time. 
...
However, as soon as the timing constraints are met, there are no additional benefits to continue improving the execution speed of the computational nodes, as the system time steps are synchronized with sensors that interface with the physical world and execution is real-time along the engine. 
...
For these examples, we used a mapping heuristic that grouped nodes requiring the same computations onto the same core. 
...
The PTARM architecture provides deterministic execution time for each instruction implemented, and the full list of instruction execution cycles are listed in table~\ref{table:ptarm_instruction_timing}.
...
We include the fixed point configuration for reference purposes only; it doesn't contain any floating point units.
...
This can be explained by the architecture of multi-threaded processors. 
...
For example, if we were limited to using five threads for each core in order to meet the timing constraint, then additional cores would be needed to fully implement our 237 node fuel rail example.     
...
The actual resource impact can be seen from Table~\ref{table:1dcfd-example_results}, which shows the total slices occupied of the three examples after place and route.
...
This proved to be critical for the 234-node common rail example, as only our heterogeneous architecture could implement the design on the XC6VLX195T FPGA; the homogeneous design did not fit.
