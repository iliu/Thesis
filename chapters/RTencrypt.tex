...
This prevents attackers from exploit shared resources from the pipeline to initiate timing side-channel attacks.    
...
In the following section we will show results of two encryption algorithms running on the cycle accurate simulator of the PRET architecture described in lickly et al~\cite{pret_cases08}.
...
It employs six threads on a six stage thread-interleaved pipeline, and also use scratchpads for an expose memory hierarchy.
Programs are written in C and compiled using a standard gcc cross compiler from Gaisler research labs~\cite{GAISLER}.
Because the results of these experiments have yet to be ported to the newer PTARM, we present the original measurements obtained on the SPARC realization of a PRET architecture.  

The SPARC PRET architecture implements a simple processor extension inspired by Ip and Edwards~\cite{ip2006processor} that adds timing instructions to the ISA.
