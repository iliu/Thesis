...
\begin{smalltabular}{  l  c || l | c | c | }
  ...
  \multicolumn{5}{|p{.9\textwidth}|}{\scriptsize{$^{\delta}$: The single store buffer (described in section~\ref{sec:ptarm_dram_store_buffer}) can hide the store latency to DRAM down a 1 thread cycle. But in cases where the store buffer cannot be used, the latency is 2 thread cycles.}} \\
