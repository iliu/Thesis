...
\begin{smalltabular}{ | p{4cm} | c || p{9cm} | }
  ...
  Move immediate to status register & 1 & \\    \cline{1-2}
  get\_time  & 2 &  \\ \cline{1-2}
  delay\_until  & 1 $^{\sigma}$ & \multirow{3}{9cm}{ ${\Delta}$: $N_{reg}$ is number of registers specified in the instruction. $L_{mem}$ is the latency of the memory region operated on.} \\    \cline{1-2}
  delay\_and\_set  & 2 $^{\sigma}$ &   \\   \cline{1-2}
  branch\_expired & 1 &  \\    \cline{1-2}
  deactive\_exception & 1 &  \multirow{2}{9cm}{${\sigma}$: denotes the minimum execution time. Actual execution time depends on the system clock and the specified deadline.} \\ \cline{1-2}
...
Worst-case execution time (WCET) is a combined analysis of the control paths that the software might exhibit, and the time it takes to execute those paths on the underlying architecture. 
...
%give example timing analysis??  

\begin{comment}
Data-processing and branch instructions are straight forward and take only one thread cycle to complete. 
Load and store instruction execution times depend on the memory access region and the addressing mode.
The scratchpads and boot ROM have single cycle access latency, while the worst-case DRAM access latency is 4 thread cycles.
\todo{Talk about store instructions.}
Load/store multiple instruction execution time depend on the number of registers specified in the instruction, and the memory region it is accessing.
If these instruction use the pre or post-index offset in their addressing mode, an additional cycle is required to update the base register.
\end{comment}  
...
The soft core build of PTARM does not yet include the DMA units for each thread.
