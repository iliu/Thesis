...
%The PC adder not only increments the PC by 4 to get the potential next PC, but it also increments the current PC by 8 to be used as an operand. 
...
\todo{mention that stack stored is on scratchpad for optimization}
...
\todo{mention that stack stored is on scratchpad, so setup instructions don't access DRAM}
