...
We implemented a DVI controller application similar to the one presented in~\cite{ip2006processor}, where a vga controller is managed purely in software through the deadline instructions presented in the paper.
...
For example, the DMA units for each thread mentioned in section~\ref{sec:ptarm_dram_integration} that provide threads with the ability to transfer contents from the DRAM to the scratchpads in the background is currently only implemented on the simulator. 
...
\begin{figure}[h]
  \begin{center}
  \includegraphics[scale=.55]{data/wcet_throughput.pdf}
  \end{center}
  \caption{Malardalen benchmarks throughput}
  \label{fig:wcet_throughput}
\end{figure}
...
\begin{figure}[h]
  \begin{center}
  \includegraphics[scale=.55]{data/wcet_latency.pdf}
  \end{center}
  \caption{Malardalen benchmarks latency}
  \label{fig:wcet_latency}
\end{figure}
